...
/*   Updated: 2019/05/14 14:46:27 by allallem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

int		ft_special_address_dprintf(unsigned long long int n, char c, t_pfspe *p)
{
	if (n == 0 && p->field <= 0 && p->precision <= 0 && c != 'p')
	{
		(p->point == 0 || ((c == 'o' || c == 'O')
		&& (p->point == 0 || p->diese > 0))) ? ft_check_dbuffer(p, '0') : 0;
		return (0);
	}
	return (1);
}
...
void	ft_adjustaddress_dprintf(t_pfspe *p, char c, char *str)
{
	if (c == 'p' || (c == 'X' && p->diese > 0) || (c == 'x' && p->diese > 0))
	{
		...
		if (c == 'X')
			ft_check_dbuffer(p, 'X');
		else
		{
			ft_check_dbuffer(p, 'x');
			(c == 'p' && ft_atoi_longlongint(str) == 0 && p->point == 0) ?
				ft_check_dbuffer(p, '0') : 0;
		}
	}
